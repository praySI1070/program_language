...
def facility_ticket_detail(facility_id):
    print(f"get team reserve data 테스트 : {get_team_reserve_data(facility_id)}")
    print(f"팀 아닌 개인 입장 테스트 : {get_weekday_by_id(facility_id)[0]}")
...
@app.route('/notices')
def notices():
    return render_template('test-map-calindar.html')
