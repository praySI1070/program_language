...
def count_now_use(facility_id):
    conn = connect_db()
    try:
        with conn.cursor() as cursor:
            a = datetime.now()
            nowtime = a.replace(microsecond=0)
            SQL = """SELECT count(*) FROM qrdata WHERE qrdata.facility_name = (SELECT 시설정보.facilities FROM 시설정보 WHERE 시설정보.code = %s)
                    and (%s > qrdata.buy_to and %s < qrdata.buy_end);"""
            cursor.execute(SQL, (facility_id, nowtime, nowtime))
            qrdata_all_from_id = cursor.fetchone()
    except Exception as e:
        print('error all_qrdata from id : ', e)
        qrdata_all_from_id = []
    return qrdata_all_from_id[0]
...
def get_notices_info(type_notice):
    conn = connect_db()
    try:
        cursor = conn.cursor()
        list_data = []
        
        SQL = "SELECT ID,제목,만든날짜,내용,facilities FROM 체육관공지 order by ID DESC"
        cursor.execute(SQL)
        row = cursor.fetchall()
        list_data = []
        for info in row:
        ...
        print('error get_notices_info :', e)
...
def get_admin(id):
    ...
    try:
        ...
        SQL = f"SELECT 관리자 FROM 사용자관리 WHERE ID = '{id}'"
        cursor.execute(SQL)
        row = cursor.fetchone()
        return row[0]
    except Exception as e:
        print('error get_admin :', e)
...
def save_notices(data):
    ...
    try:
        ...
        SQL = f"INSERT INTO 체육관공지 VALUES (0,'{data['title']}',now(),'{data['detail']}','{data['notification_type']}')"
        cursor.execute(SQL)
        conn.commit()
        return
    except Exception as e:
        print('error save_notices :', e)

def delete_notices_SQL(data):
    conn = connect_db()
    try:
        cursor = conn.cursor()
        for info in data['dates']:
            SQL = f"DELETE FROM 체육관공지 WHERE 만든날짜 = '{info}'"
            print(info)
            cursor.execute(SQL)
            conn.commit()
        return
    except Exception as e:
        print('error delete_notices_SQL :', e)
...
def update_notices(data):
    ...
    try:
        ...
        SQL = f"UPDATE 체육관공지 SET 제목 = '{data['title']}', 내용 = '{data['detail']}', facilities = '{data['notification_type']}' WHERE 만든날짜 = '{data['date']}'"
        cursor.execute(SQL)
        conn.commit()
        return
    except Exception as e:
        print('error update_notices :', e)
